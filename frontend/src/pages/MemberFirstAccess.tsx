...
export function MemberFirstAccess({onBack,onComplete}:{onBack:()=>void;onComplete:()=>void}){
  ...
  const submit=(event:React.FormEvent)=>{event.preventDefault();setError('');if(password!==confirmation){setError('As senhas não coincidem.');return}setBusy(true);api.registerMember({name,email,invite_code:inviteCode,password,password_confirmation:confirmation}).then(()=>setDone(true)).catch(err=>setError(err instanceof Error?err.message:'Não foi possível concluir o primeiro acesso')).finally(()=>setBusy(false))};
